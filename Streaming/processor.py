...
def process_products(unprocessed_path, processed_path, best_product) :
    # Can add False color if needed
    unzip(unprocessed_path, best_product.iloc[0]['identifier'])

    imagePath = '{}/{}.SAFE/GRANULE/'.format(unprocessed_path, best_product.iloc[0]['identifier'])
    ...
    trueColor = rio.open('Streaming/data/TC/{}_tc.tiff'.format(best_product.iloc[0]['identifier']), 'w', driver='Gtiff',
